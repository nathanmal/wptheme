...
class Integrations extends Page
{
  ...
  public function metabox_maps()
  {
    ...
    $this->render_setting( 'maps.google.apikey','text','Google Maps API Key', $config);

    $key = Setting::get('maps.google.apikey');

    if( ! empty($key) )
    {
      $config = array('note'=>'Set the default map center');

      $this->render_setting('maps.center','latlong','Map Center', $config);

      $config = array('note'=>'Set the default map zoom level');

      $this->render_setting('maps.zoom','integer', 'Zoom Level', $config);
    }
    ...
    $this->render_setting( 'maps.snazzymaps.apikey', 'text', 'Snazzymaps API Key', $config);

    $skey = Setting::get('maps.snazzymaps.apikey', FALSE);

    if( ! empty($skey) )
    {
      $this->render_setting('maps.snazzymaps.style','snazzymaps','Map Style');
    }
  ...
  public function metabox_analytics()
  {
    ...
    $this->render_setting('google.analytics.ua','text','Tracking ID', $config);
  }
  ...
  public function render()
  {
    ...
    ?> 
     <section id="wpt-google-maps">
      ...
      $this->render_setting( 'google.maps.apikey','text','Google Maps API Key', $config);
      ...
      if( ! empty($api_key) )
      {
        $this->render_setting( 'google.maps.center', 'latlong', 'Map Center');

        $this->render_setting( 'snazzymaps.apikey', 'text', 'Snazzy Maps API Key' );
        ...
        $this->render_setting( 'google.maps.style', 'json', 'Map Style', $config );
    ...
    <section id="wpt-google-analytics">
      ...
      <?php 
      $this->render_setting( 'gogole.analytics.code','text', 'Google Analytics Code', array('placeholder'=>'UA-XXXXXXX'));
      ?>
    </section>
